...
This page covers how to customize the AI behavior in **VectraDocs**. The AI uses direct OpenAI-compatible API calls and **Orama** for client-side vector search.
...
4. **Generate response** → Direct API call sends the question + context to the LLM
...
Find the `systemPrompt` variable:
...
<Callout title="Optimized for Speed" type="info">
As of v1.0.2, VectraDocs uses direct API calls instead of LangChain for ~200-500ms faster responses.
</Callout>
